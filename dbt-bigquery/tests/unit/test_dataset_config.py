...
class TestBigQueryReplicationConfig:
    ...
    def test_full_config(self):
        ...
        config = BigQueryReplicationConfig(
            enabled=True, replicas=["us-east1", "us-west1"], primary_location="us-central1"
        )
        assert config.enabled is True
    ...
    def test_to_dict(self):
        ...
        config = BigQueryReplicationConfig(
            enabled=True, replicas=["us-east1"], primary_location="us-central1"
        )
...
class TestBigQueryDatasetConfig:
    ...
    def test_from_dict_without_replication(self):
        ...
        data = {"location": "EU", "labels": {"env": "dev"}}
    ...
    def test_validate_replication_no_replicas(self):
        ...
        replication = BigQueryReplicationConfig(
            enabled=True, replicas=[], primary_location="us-central1"
        )
        config = BigQueryDatasetConfig(name="test", location="US", replication=replication)
        errors = config.validate()
        assert len(errors) > 0
    ...
    def test_validate_replication_no_primary(self):
        ...
        replication = BigQueryReplicationConfig(
            enabled=True, replicas=["us-east1"], primary_location=None
        )
        config = BigQueryDatasetConfig(name="test", location="US", replication=replication)
        errors = config.validate()
        assert len(errors) > 0
...
class TestDatasetConfigManager:
    ...
    def test_parse_simple_configs(self):
        ...
        datasets_config = {
            "analytics": {"location": "US", "labels": {"env": "prod"}},
            "staging": {"location": "EU"},
        }
    ...
    def test_get_all_configs(self):
        ...
        datasets_config = {"analytics": {"location": "US"}, "staging": {"location": "EU"}}
        manager = DatasetConfigManager(datasets_config)
    ...
    def test_validate_all_with_errors(self):
        ...
        datasets_config = {
            ...
            "staging": {"location": ""},  # Empty location
        }
        manager = DatasetConfigManager(datasets_config)
        errors = manager.validate_all()
        assert len(errors) > 0
