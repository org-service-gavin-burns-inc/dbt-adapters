    ...
    def validate(self) -> List[str]:
        ...
        if self.has_replication():
            if not self.replication.replicas:
                errors.append(f"Dataset '{self.name}': replication enabled but no replicas specified")
            
            if not self.replication.primary_location:
                errors.append(f"Dataset '{self.name}': replication enabled but no primary_location specified")
            
            # Validate replica locations are valid
            for replica in self.replication.replicas:
                if not replica or not isinstance(replica, str):
                    errors.append(f"Dataset '{self.name}': invalid replica location: {replica}")
